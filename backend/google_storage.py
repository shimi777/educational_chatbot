...
Google Sheet structure:
  ...
  - "Feedback" worksheet: student questionnaire responses (all numeric except Q12)
...
class GoogleStorage:
    ...
    def __init__(self, spreadsheet_id: str, drive_folder_id: str = ""):
        ...
        self._feedback_ws = self._spreadsheet.worksheet(_WS_FEEDBACK)
    ...
    def save_feedback(
        ...
        """Append a student feedback row to the Feedback worksheet.

        All q1-q11 values are integers. Q12 is free text.
        Multiple-choice key (not shown to students):
          Q2: 1=regularly, 2=sometimes, 3=first time
          Q7: 1=didn't use, 2=when stuck, 3=after every explanation, 4=to plan
        """
        row = [
            session_id,
            datetime.now().isoformat(),
        ...
        self._feedback_ws.append_row(row, value_input_option="RAW")
        logger.info("Feedback saved | session=%s lang=%s", session_id, lang)
