...
def _reset_to_join() -> None:
    for key in [
        ...
        "session_loaded", "session_id_input", "feedback_submitted",
    ]:
        ...
        elif key in ("session_ended", "lesson_auto_advanced", "retro_initialized",
                      "confirm_eval_pending", "feedback_submitted"):
...
def _save_feedback_to_cloud(responses: dict) -> None:
    from backend.config import config as app_config
    if not app_config.google_spreadsheet_id:
        return
    try:
        ...
        logger.error("Feedback save failed (non-blocking): %s", exc)
...
def _render_feedback_form() -> None:
    ...
    st.subheader(t(
        ...
        "(דקות 3) משוב מהיר — עזרו לנו להשתפר",
    ))
    ...
    with st.form("feedback_form"):
        ...
        st.markdown(t(
            ...
            "**1.** האפליקציה הייתה קלה לשימוש",
        ))
        q1 = st.select_slider(
            t("Very difficult → Very easy", "קשה מאוד → קלה מאוד"),
            options=[1, 2, 3, 4, 5], value=3, key="fb_q1",
        )
        ...
        # -- Q2 --
        st.markdown(t(
            '**Q2.** Have you ever tried "learning by teaching" before?',
            "**2.** ?האם ניסית בעבר ללמוד דרך הוראה",
        ))
        q2_options = [
            t("Yes, I regularly explain concepts to others to learn",
              "כן, אני מסביר/ה מושגים לאחרים באופן קבוע כדי ללמוד"),
            t("Sometimes, but not intentionally",
              "לפעמים, אך לא בכוונה"),
            t("No, this was my first time",
              "לא, זו הייתה הפעם הראשונה שלי"),
        ]
        q2_label = st.radio(
            t("Select one", "בחר/י אחד"), options=q2_options,
            index=None, key="fb_q2",
        )
        q2 = q2_options.index(q2_label) + 1 if q2_label else None

        st.divider()

        # -- Q3 --
        st.markdown(t(
            "**Q3.** How familiar were you with the topic BEFORE using the app?",
            "**3.** ?כמה מוכר לך הנושא לפני השימוש באפליקציה",
        ))
        q3 = st.select_slider(
            t("Never heard of it → I could teach it to others",
              "מעולם לא שמעתי על זה → יכולתי ללמד את זה לאחרים"),
            options=[1, 2, 3, 4, 5], value=3, key="fb_q3",
        )
        ...
        # -- Q4 --
        st.markdown(t(
            '**Q4.** The "struggling student" felt realistic and engaging',
            '**4.** ה"תלמיד המתקשה" הרגיש מציאותי ומעניין',
        ))
        q4 = st.select_slider(
            t("Strongly disagree → Strongly agree",
              "לא מסכים/ה בכלל → מסכים/ה מאוד"),
            options=[1, 2, 3, 4, 5], value=3, key="fb_q4",
        )
        ...
        # -- Q5 --
        st.markdown(t(
            "**Q5.** I felt challenged to think deeply about the topic",
            "**5.** הרגשתי שאותגרתי לחשוב לעומק על הנושא",
        ))
        q5 = st.select_slider(
            t("Strongly disagree → Strongly agree",
              "לא מסכים/ה בכלל → מסכים/ה מאוד"),
            options=[1, 2, 3, 4, 5], value=3, key="fb_q5",
        )
        ...
        # -- Q6 --
        st.markdown(t(
            '**Q6.** Teaching the "student" helped me find gaps in my own understanding',
            '**6.** הוראת ה"תלמיד" עזרה לי לגלות פערים בהבנה שלי',
        ))
        q6 = st.select_slider(
            t("Strongly disagree → Strongly agree",
              "לא מסכים/ה בכלל → מסכים/ה מאוד"),
            options=[1, 2, 3, 4, 5], value=3, key="fb_q6",
        )
        ...
        # -- Q7 --
        st.markdown(t(
            "**Q7.** How did you primarily use the mentor?",
            "**7.** ?איך השתמשת בעיקר במנטור",
        ))
        q7_options = [
            t("I didn't use the mentor at all",
              "לא השתמשתי במנטור כלל"),
            t("Only when I got stuck",
              "רק כשנתקעתי"),
            t("After every explanation to check my approach",
              "אחרי כל הסבר כדי לבדוק את הגישה שלי"),
            t("At the beginning to plan my teaching strategy",
              "בהתחלה כדי לתכנן את אסטרטגיית ההוראה שלי"),
        ]
        q7_label = st.radio(
            t("Select one", "בחר/י אחד"), options=q7_options,
            index=None, key="fb_q7",
        )
        q7 = q7_options.index(q7_label) + 1 if q7_label else None

        st.divider()

        # -- Q8 --
        st.markdown(t(
            "**Q8.** The mentor's advice was helpful for improving my explanations",
            "**8.** העצות של המנטור עזרו לי לשפר את ההסברים שלי",
        ))
        q8 = st.select_slider(
            t("Strongly disagree → Strongly agree",
              "לא מסכים/ה בכלל → מסכים/ה מאוד"),
            options=[1, 2, 3, 4, 5], value=3, key="fb_q8",
        )
        ...
        # -- Q9 --
        st.markdown(t(
            "**Q9.** The evaluation feedback was fair and useful",
            "**9.** המשוב על הביצועים היה הוגן ושימושי",
        ))
        q9 = st.select_slider(
            t("Strongly disagree → Strongly agree",
              "לא מסכים/ה בכלל → מסכים/ה מאוד"),
            options=[1, 2, 3, 4, 5], value=3, key="fb_q9",
        )
        ...
        # -- Q10 --
        st.markdown(t(
            "**Q10.** After using the app, I understand the topic better",
            "**10.** אחרי השימוש באפליקציה, אני מבין/ה את הנושא טוב יותר",
        ))
        q10 = st.select_slider(
            t("Strongly disagree → Strongly agree",
              "לא מסכים/ה בכלל → מסכים/ה מאוד"),
            options=[1, 2, 3, 4, 5], value=3, key="fb_q10",
        )
        ...
        # -- Q11 --
        st.markdown(t(
            "**Q11.** I would recommend this app to others for learning",
            "**11.** הייתי ממליץ/ה על האפליקציה הזו לאחרים ללמידה",
        ))
        q11 = st.select_slider(
            t("Strongly disagree → Strongly agree",
              "לא מסכים/ה בכלל → מסכים/ה מאוד"),
            options=[1, 2, 3, 4, 5], value=3, key="fb_q11",
        )
        ...
        # -- Q12 --
        st.markdown(t(
            "**Q12.** Any other feedback? (Optional)",
            "**(אופציונלי) ?משוב נוסף .12**",
        ))
        q12 = st.text_area(
            t("What worked well? What could be improved?",
              "מה עבד טוב? מה אפשר לשפר?"),
            key="fb_q12", max_chars=1000,
        )

        submitted = st.form_submit_button(
            t("Submit Feedback", "שלח משוב"),
            type="primary", use_container_width=True,
        )

        if submitted:
            if q2 is None or q7 is None:
                st.error(t(
                    "Please answer all required questions (Q2 and Q7).",
                    ".(7-ו 2 שאלות) הנדרשות השאלות לכל לענות יש",
                ))
            else:
                responses = {
                ...
                _save_feedback_to_cloud(responses)
